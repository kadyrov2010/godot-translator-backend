...
**Эффект от кеша:**
- Популярные слова переводятся мгновенно (0.01 сек)
- Снижение нагрузки на Gemini API (~30-50% меньше запросов)
- Защита от превышения лимитов (15 запросов/мин)
...
| **Gemini API calls** | 100% | **50-70%** (остальное из кеша) |
...
### Проблема 3: Gemini 429 "Too Many Requests"
**Причина:** Превышен лимит 15 запросов/минуту

**Решение с кешем:**
- Кеш автоматически снижает количество запросов
- Популярные слова (`hello`, `cat`, `food`) берутся из кеша
- Реальных запросов к Gemini станет в 2 раза меньше
